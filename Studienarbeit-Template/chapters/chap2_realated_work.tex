...
\section*{Prior Work on Connections Solving}
...
\section*{Word Puzzles as weighted \ac{CSP}}
...
Applying this concept to word puzzles is not new. \citeauthor{ginsbergDrFillCrosswordsImplemented2014} formalizes American-style crossword solving as a weighted \ac{CSP} in the program Dr\.Fill: each grid cell is a variable, each candidate fill is a value, and the constraints require intersecting words to agree on shared letters \cite{ginsbergDrFillCrosswordsImplemented2014}. Because many fills are plausible for a given clue, each candidate fill is given a likelihood-based weight, and Dr.Fill searches for the assignment that maximizes total weight while still satisfying the grid's hard constraints. The weights are drawn from several sources: a database of over 47,000 previously published crosswords with 1.9 million unique clues, two dictionaries of varying size and "crossword merit," 1.2 million synonyms drawn from WordNet and other online sources, and a database of Wikipedia titles and word pairs used to identify proper nouns and complete fill-in-the-blank clues. These sources feed into a scoring function that combines five weighted criteria, including clue-history matching, part-of-speech agreement, and crossword "merit," into a single likelihood score for each candidate fill. Dr.Fill then searches for the assignment that maximizes total score while still satisfying the grid's hard letter-intersection constraints. Two further design choices in Dr.Fill are worth noting. First, they found that looking further ahead before committing to a choice gave no real benefit, and the system used only a single step of lookahead instead. Second, branch-and-bound, a standard pruning technique for weighted CSPs, was tested and dropped because it added little value in this domain, as a result postprocessing was used instead. In both cases, a more sophisticated search method did not beat a simpler one, once the scoring function and hard constraints were doing most of the work. Behind both choices is a wider point: complete knowledge of what each clue means is impractical to obtain, so Dr.Fill relies mainly on the hard constraint that crossing words must share letters to narrow down candidates, not on understanding the clues themselves. The structure of the problem, not knowledge of its content, does most of the work.

% set partitioning
The constraint $C$ defined above, that every word belongs to exactly one group and every group contains exactly four words, is a combinatorial optimization problem known as set partitioning. A partition of a set is a way of dividing all of its elements into non-empty, pairwise disjoint subsets whose union is the original set \cite{PartitionSet2026}. The set partitioning problem asks, given a ground set and a collection of candidate subsets each associated with a cost, to select a subset of these candidates that forms a partition of the ground set at minimum or maximum total cost \cite{muellerSolvingSetPartitioning1998}. A classic real-world instance is airline crew scheduling, where pilots and flight attendants must be assigned to crews so that every flight is covered by exactly one crew, at minimum cost \cite{muellerSolvingSetPartitioning1998}. Connections maps onto this same structure, with the sixteen puzzle words as the ground set, candidate four-word groups as the subsets among which a selection must be made, and each candidate group's score from the weighted \ac{CSP} above serving as its cost. Identifying this structure matters because set partitioning problems admit exact solution methods, in contrast to the greedy or heuristic-clustering approaches commonly used for Connections in prior work (\autoref{sec:prior-work-connections}).
