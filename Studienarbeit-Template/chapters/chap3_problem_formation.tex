...
\label{eq:group-def}
\end{equation}
\eqcaption{Group membership, $G_k = \{w_i : x_i = k\}$}
...
\end{equation}
\eqcaption{Group size constraint, $|G_k| = 4$ for every $k$}
...
\end{equation}
\eqcaption{Weighted CSP objective, $x^* = \argmax_x \sum_{k=1}^{4} s(G_k)$}
...
\end{equation}
\eqcaption{Number of valid partitions, $|\mathcal{H}| = \frac{16!}{(4!)^4 \cdot 4!} = 2{,}627{,}625$}
...
\end{equation}
\eqcaption{Prior entropy over all partitions, $H_{\text{max}}(\mathcal{H}) = \log_2 |\mathcal{H}| \approx 21.3$ bits}
...
\end{equation}
\eqcaption{Information per feedback response, $\log_2 3 \approx 1.58$ bits}
...
\end{equation}
\eqcaption{Maximum information obtainable from feedback, $4 \log_2 3 \approx 6.34$ bits}
