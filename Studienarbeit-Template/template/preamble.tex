...
\addbibresource{etc/IEEEabrv.bib}
\addbibresource{etc/MYabrv.bib}
...
% add package for styling metrics table 
\usepackage{booktabs}
\usepackage{array}
...
\definecolor{perfblue}{HTML}{185FA5}
\definecolor{relgreen}{HTML}{3B6D11}
\definecolor{opsamber}{HTML}{854F0B}
\definecolor{costpink}{HTML}{993556}
\definecolor{headergray}{HTML}{F1EFE8}
...
% Footnotes inside table/figure floats (plain \footnote loses its text
% when issued inside a float, since the float can move to another page)
...
\cftsetindents{equations}{0pt}{2.3em}
\newcommand{\eqcaption}[1]{\addcontentsline{loe}{equations}{\protect\numberline{\theequation}#1}}
...
%% Global capitalized autoref names — defined after all packages load
\AtBeginDocument{%
...
%% Numbered sections/subsections that are kept out of the table of contents
%% (heading still gets its number, e.g. "2.3", just no ToC entry).
